...
public class API {
	...
	private double time; // transportation time
	
	public API() {
		this.globalagentloc = "7QQ33G8H%2BRX5"; // ¥ý¹w³]
	}
	
	/**
	 * This method converts the latitude and longitude of the position to global
	 * code the output is suited for api request format
	 * 
	 * @param storePosition - longitude and latitude informations of certain position in
	 *                 class {@code Position}.
	 * @return - API format of informations.
	 */
	public String toGlobal(Position storePosition) {
		
		String globalLoc = null;
		String lat = Double.toString(storePosition.getLatitude()); // latitude in String.
		String lng = Double.toString(storePosition.getLongtitude()); // longitude in String.
		String APIkey = "AIzaSyDoUlM151mrLPY77zLQKSzcUM89NzpJOfs";
		String message = "https://maps.googleapis.com/maps/api/geocode/json?latlng=" + lat + "," + lng + "&key="
				+ APIkey;
	 ...
	 * 
	 * @param strarr - String array in API request format, which is
	 *               converted by method {@code toGlobal()}.
	...
	public int travelTime(String[] strarr) {
		...
		for (int i = 0; i < strarr.length; i++) {
			...
			String message = "https://maps.googleapis.com/maps/api/distancematrix/json" + "?destinations=" + globaldes
					+ "&origins=" + this.globalagentloc + "&key=" + APIkey;
			try {
				URL url = new URL(message);
				HttpURLConnection conn = (HttpURLConnection) url.openConnection();
				conn.setRequestMethod("GET");
